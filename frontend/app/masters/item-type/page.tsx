...
import { itemTypeApi, ItemTypeRow } from "@/lib/api-services";
...
export default function ItemTypePage() {
  const [data,      setData]      = useState<ItemTypeRow[]>([]);
  const [loading,   setLoading]   = useState(true);
  const [saving,    setSaving]    = useState(false);
  const [error,     setError]     = useState<string | null>(null);
  const [showModal, setShowModal] = useState(false);
  const [editItem,  setEditItem]  = useState<ItemTypeRow | null>(null);
  const [deleteId,  setDeleteId]  = useState<number | null>(null);
  const [viewItem,  setViewItem]  = useState<ItemTypeRow | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try { setData((await itemTypeApi.list()).items); }
  ...
  function openAdd() {
    setEditItem(null); setFName(""); setFDesc(""); setFActive(true);
    setShowModal(true);
  }

  function openEdit(item: ItemTypeRow) {
    setEditItem(item);
    setFName(item.name);
    setFDesc(item.description ?? ""); setFActive(item.isActive);
    setShowModal(true);
  }
  ...
  async function handleSave() {
    ...
    try {
      if (editItem) {
        const u = await itemTypeApi.update(editItem.id, {
          name: fName.trim(),
          description: fDesc.trim() || undefined, isActive: fActive,
        });
        setData((prev) => prev.map((x) => x.id === u.id ? u : x));
      } else {
        const c = await itemTypeApi.create({ name: fName.trim(), description: fDesc.trim() || undefined });
        setData((prev) => [...prev, c]);
      }
      setShowModal(false);
  ...
  const columns: ColumnConfig<ItemTypeRow>[] = useMemo(() => [
    {
      id: "name", accessorKey: "name", header: "Item Type",
  ...
  return (
    <div className="space-y-5 pb-24">
      ...
      <DataGrid
        data={data} columns={columns} tableName="item-types"
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)}
        title={editItem ? "Edit Item Type" : "Add Item Type"} size="sm"
        footer={
          <>
        ...
        <div className="rounded-xl bg-white p-4 shadow-sm space-y-3.5">
          <div>
            <label className={labelCls}>Item Type Name *</label>
            <input value={fName} onChange={(e) => setFName(e.target.value)} placeholder="e.g. Sheet" className={inputCls} />
          </div>
          <div>
      ...
      {viewItem && (
        <RowDetailModal isOpen={!!viewItem} onClose={() => setViewItem(null)} title="Item Type Details"
          fields={[
      ...
      <Modal isOpen={!!deleteId} onClose={() => setDeleteId(null)} title="Delete Item Type" size="sm"
        footer={
          <>
